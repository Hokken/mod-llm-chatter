...
Last updated: 2026-03-12 (through Session 68 - achievement batching,
talent injection, humor hints, and ambient pacing updates)
...
- the module has remained in active runtime use after that validation,
  and this document reflects the current shipped architecture through
  Session 68
...
- delivery order is `ORDER BY deliver_at ASC LIMIT 1`
- once a message is inserted here, event priority is no longer used;
  delivery is purely time-based
...
The current system does **not** implement a unified end-to-end priority
manager across all queues. In particular:

- `priority` only affects fetch order from `llm_chatter_events`
- `llm_chatter_queue` is FIFO and has no priority field
- `llm_chatter_messages` delivers by `deliver_at`, not by originating
  event priority
- `GlobalMessageCap` and `TransportBypassGlobalCap` are currently loaded
  and logged, but not enforced by the active runtime path

This matches the code and is why future planning notes still call out a
priority-system improvement as unfinished work.
...
This matters for a future priority redesign because priority today is
applied only at event claim time, not across every kind of worker task.
...
- transport state and route announcements
