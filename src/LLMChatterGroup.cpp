...
static void CleanupGroupSession(uint32 groupId)
{
    ...
    auto itQ = _questAcceptTimestamps.begin();
    while (itQ != _questAcceptTimestamps.end())
    {
        if ((itQ->first >> 32) == groupId)
            itQ = _questAcceptTimestamps.erase(itQ);
        else
            ++itQ;
    }
...
public:
        ...
        uint8 xpSource) override
    {
        ...
        // Suppress discovery in raid/BG
        {
            Map* dMap = player->GetMap();
            if (dMap
                && (dMap->IsRaid()
                    || dMap->IsBattleground()))
                return;
        ...
        if (!area)
            return;
        char const* an = area->area_name[
            sWorld->GetDefaultDbcLocale()];
        std::string areaName = an ? an : "";
        if (areaName.empty())
        {
            an = area->area_name[LOCALE_enUS];
            areaName = an ? an : "";
        }
        if (areaName.empty())
            return;
