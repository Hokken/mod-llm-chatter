-- Add duel start and duel end group events.
...
SET @has_duel_events = (
  ...
  WHERE `TABLE_SCHEMA` = DATABASE()
    ...
    AND `COLUMN_TYPE` LIKE '%bot_group_duel_end%'
);
...
SET @sql = IF(
  ...
  CONCAT(
    ...
    "'proximity_player_emote','proximity_boss_approach',",
    "'proximity_boss_player_say','bot_backstory_regen',",
    "'bot_tone_regen','guild_idle_chatter',",
    "'guild_player_message','guild_login_greeting',",
    "'bot_group_duel_start','bot_group_duel_end') NOT NULL"
  ),
  'SELECT 1'
);
